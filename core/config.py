...
Role as runtime-config defaults:
    Operational params here (WINDOW, Z_ENTRY, Z_ATTENTION, BUY/SELL SL/TP/BE,
    ENTRY/FORCE_CLOSE_*, Z_ANOMALY, RHO_BREAKDOWN_LEVEL, BETA_DELTA_MAX,
    EG_PVALUE_THRESHOLD) seed ``core/runtime_config.DEFAULTS``. Engine,
    risk_gate, signals, and replay all read from the runtime profile at call
    time; these globals only kick in when callers pass ``None`` (legacy code
    paths) or run isolated tests/scripts. Operators tune via
    ``POST /api/runtime-config`` — do not edit this file to change live
    behavior.
"""
import os
