...
class TradeEngine:
    ...
    def count_trades_today(self, date_str: str, *, live_only: bool = False) -> int:
        ...
        if live_only:
            where += " AND live = 1"
        n = conn.execute(
            f"SELECT COUNT(*) FROM matador_ops WHERE {where}",
            (date_str,)
        ).fetchone()[0]
        conn.close()
        return int(n)

    def pnl_today(self, date_str: str, *, live_only: bool = False) -> float:
