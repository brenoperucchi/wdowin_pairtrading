...
- `current_live_issue(db_path)` — última falha crítica recente e não recuperada com `closed_bar_ts IS NULL` (ex.: `MT5_DISCONNECTED`).
...
- `current_live_issue` retorna evento crítico recente sem barra, expira falhas antigas e é limpo por evento DATA posterior com status não-FAILED
...
- [x] #6 `current_live_issue` retorna falha crítica recente com closed_bar_ts NULL, expira falhas antigas e é limpo por recuperação DATA
...
- `tests/test_execution_timeline.py` com 19 testes cobrindo: idempotência do schema, WAL pragma após `_init_db`, distance/ratio para operadores `>` e `<`, dedupe por collision, bulk transação + payload_json dict serializado, filtros `phase/status/strategy/event/since/limit`, clamp de `limit`, ordem do funil em `current_bottleneck`, MAX(closed_bar_ts) usado, None quando barra passa limpa, `current_live_issue` retorna FAILED recente sem barra, expira falhas antigas, limpa por recuperação DATA e ignora eventos com barra, validação de campos obrigatórios.

`PYTHONPATH=/tmp/codex-pytest python3 -m pytest tests/ -q` → 165 passed (19 novos + 146 existentes).
<!-- SECTION:NOTES:END -->
