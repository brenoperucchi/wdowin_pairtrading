...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Form de filtros tem `mode=live|replay` (radio/select) e `date` (input date), enviados via GET.
- [x] #2 Modo live (default) mantém visual e auto-refresh atuais.
- [x] #3 Modo replay com data válida e DB existente mostra eventos do DB de replay; banner indica `Replay YYYY-MM-DD`; meta-refresh desabilitado.
- [x] #4 Modo replay com DB inexistente renderiza página com mensagem amigável ("Sem replay para esta data") em vez de 404/500.
- [x] #5 Handler usa o mesmo path/validação do Slice C (sem duplicar regex de data).
- [x] #6 QueryString preserva o estado: clicar Apply em replay mantém `mode=replay&date=...`.
<!-- AC:END -->

## Definition of Done
<!-- DOD:BEGIN -->
- [x] #1 Subir o servidor, abrir `http://localhost:8080/execution-timeline?mode=replay&date=2026-05-08`, conferir banner e tabela.
- [x] #2 Conferir manualmente que `mode=live` continua exibindo dados do `trades.db` em tempo real.
<!-- DOD:END -->
