...
def run_simulation():
    ...
    def simulate_curve(target_zb, target_zs, use_filter):
        pos_buy = None
        pos_sell = None
        ...
        for i in range(WINDOW, n):
            ...
            if t >= ENTRY_START and t <= ENTRY_END:
                blocked = use_filter and (regime == 'BULL')
                
                if pos_buy is None and not blocked:
                    if is_safe(b_b, i, r_b[i]) and z_b_val <= -target_zb:
                        pos_buy = {'px': wc[i], 'sl_level': wc[i] - BUY_SL, 'be': False}
                        
                if pos_sell is None and not blocked:
                    if is_safe(b_s, i, r_s[i]) and z_s_val >= target_zs:
        ...
        return dates_out, eq_out, cum_pnl, dd, trade_count

    # Curva 1: O "Setup Matador" Definitivo Clássico
    # (BUY Kalman = 2.0, SELL OLS = 2.1, SEM FILTRO)
    print("Simulando Setup Matador Definitivo...")
    d1, eq1, p1, dd1, t1 = simulate_curve(2.0, 2.1, False)
    rf1 = p1/dd1 if dd1 > 0 else 0
    
    # Curva 2: O Novo Agressivo "Escudo HMM"
    # (BUY Kalman = 1.80, SELL OLS = 1.80, COM FILTRO M30 BULL = BLOQUEIO)
    print("Simulando Setup Agressivo HMM (Z=1.8)...")
    d2, eq2, p2, dd2, t2 = simulate_curve(1.8, 1.8, True)
    ...
    ax.plot(d1, eq1, color='#787c8a', alpha=0.9, linewidth=1.8, linestyle='--',
            label=f"[Antigo] Matador (Z: 2.0/2.1 | Sem HMM) \nTrades: {t1}  |  PnL: R${p1:,.0f}  |  MaxDD: R${dd1:,.0f}  |  RF: {rf1:.2f}")
    
    ax.plot(d2, eq2, color='#00e87a', linewidth=2.5, 
            label=f"[NOVO] Matador HMM (Z: 1.8/1.8 | Escudado) \nTrades: {t2}  |  PnL: R${p2:,.0f}  |  MaxDD: R${dd2:,.0f}  |  RF: {rf2:.2f}")
    
    ax.set_title("O Confronto: Antigo Setup Matador vs. Nova Vanguarda com IA M30", fontsize=16, fontweight='bold', pad=20)
    ax.set_ylabel("Capital Líquido Consolidado (R$)", fontsize=13)
    ax.grid(color='#333333', linestyle=':', linewidth=0.8)
    
    leg = ax.legend(loc="upper left", fontsize=12, facecolor='#111111', edgecolor='#444444', framealpha=0.9)
    for text in leg.get_texts():
        text.set_color("white")
        
    ax.fill_between(d2, eq2, color='#00e87a', alpha=0.15)
    
    save_path = "C:/Users/ryzen/.gemini/antigravity/brain/ef5aebe9-595c-4cef-ba9a-979de966c203/final_vs_hmm.png"
    plt.tight_layout()
