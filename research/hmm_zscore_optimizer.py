...
def calc_rf(pnl_arr):
    if len(pnl_arr) == 0: return 0, 0, 0
    p = np.sum(pnl_arr)
    eq = np.cumsum(pnl_arr)
    pk = np.maximum.accumulate(eq)
    dd = (pk - eq).max()
    rf = p / dd if dd > 0 else 0
    return p, dd, rf

def simulate_with_z_grid():
    data = load_data()
    wc = data['win'].values.astype(float); dc = data['wdo'].values.astype(float)
    
    print("Recalculando Filtros e Z-Scores base...")
    ...
    # Grid search (1.5 to 2.2 step 0.1)
    z_grid = np.arange(1.5, 2.3, 0.1)
    
    results = []
    
    def block_trade(direction, z_val, regime, filter_type):
        if filter_type == "Sem Filtro":
            return False
            
        elif filter_type == "Requisicao (Nao vende BULL, Nao compra BEAR)":
            if direction == 'sell' and regime == 'BULL': return True
            if direction == 'buy' and regime == 'BEAR': return True
            return False
            
        elif filter_type == "Estatistico Puro (Cortar RF Negativo)":
            if direction == 'sell' and regime == 'BULL': return True
            if direction == 'buy' and regime == 'BULL': return True
            return False
    ...
    for z_test in z_grid:
        for ftype in ["Sem Filtro", "Requisicao (Nao vende BULL, Nao compra BEAR)", "Estatistico Puro (Cortar RF Negativo)"]:
    ...
    print("\n\n=== TABELA DE Z-SCORE E AGRESSIVIDADE ===")
    
    # Mapeando tabelas para visualização
    for ftype in df_res['Filter'].unique():
        print(f"\n--- Filtro: {ftype} ---")
        subset = df_res[df_res['Filter'] == ftype].drop(columns=['Filter'])
        print(subset.to_string(index=False, float_format="%.2f"))
        
if __name__ == '__main__':
    simulate_with_z_grid()
